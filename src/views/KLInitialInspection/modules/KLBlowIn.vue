...
import CrystalKL from './crystalKL.vue';
...
export default {
  ...
  methods: {
    ...
    async getDetail() {
      ...
      const data = sessionStorage.getItem('klcj_in_' + this.overStation.processingOrderCode)
      if (data) {
        //有上次保存数据进行数据还原
        ...
        await Api.listByPage({
          ...
          if (res.data && res.data.rows.length > 0) {
            this.detailForm = Object.assign({}, deflutFrom, this.overStation.fromData, JSON.parse(JSON.stringify(res.data.rows[0])))
          }
        })
      }
      this.$nextTick(() => {
    ...
    async saveDetail(type) {
      const form = await this.$refs.formRef.getRules(type)
      ...
      if (!type) {
        // 保存
        sessionStorage.setItem('klcj_in_' + this.overStation.processingOrderCode, JSON.stringify(saveData))
    ...
    generate(data) {
      ...
      inStation.inStation(saveData).then(res => {
        this.$message.success('提交成功')
        sessionStorage.removeItem('klcj_in_' + this.overStation.processingOrderCode)
