<!--申请弹窗-->
<template>
  <el-dialog :visible.sync="dialogFormVisible" title="更换申请" width="95%" append-to-body>
    ...
    <el-form ref="dialogForm" :model="dialogForm" label-width="100px">
      ...
      <el-form-item :rules="[{ required: true, message: '请输入', trigger: 'change' }]" label="更换原因" prop="reason">
...
import {applyAccessory} from "@/api/accessory";
...
export default {
  ...
  created() {
    getSeleteData('applyReason', this.reasonMap)
  },
  methods: {
    init() {
      ...
      this.$refs.dialogForm.validate(valid => {
        if (valid) {
          ...
          applyAccessory({
            ...
            equipmentCode: this.rowData.equipmentCode,
          }).then(res => {
