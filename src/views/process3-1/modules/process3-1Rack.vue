...
<template>
  <div class="detailBox">
    <el-form ref="detailForm" :model="detailForm" label-width="90px" :rules="rules" style="padding: 0 10px 0 0;">
      <div class="topInfoCard">
        <el-row>
          <el-col :span="12">
            <el-form-item label="退火炉编号" prop="equipmentCode" label-width="110px">
            ...
            <div>
              ...
              <el-row>
                <el-col :span="12">
                  ...
                  >
                    <el-input v-trim v-model="item.seedPosition" placeholder="请选择籽晶位置" readonly style="width: 100%;"/>
                  </el-form-item>
                </el-col>
                <!--最后一个籽晶取消“压块”的录入-->
...
const defaultForm = {
  equipmentCode: null, // 退火炉编号
  equipmentId: null,
  technology: null, // 退火炉工艺
  ...
  data() {
    return {
      ...
      rules: {
        ...
        graphiteBase: [{ required: true, message: '必填项', trigger: 'blur' }],
        lid: [{ required: true, message: '必填项', trigger: 'blur' }],
      },
      disabled: true,
      eqpList: [],
  ...
  methods: {
    ...
    init() {
      ...
      if (processingOrderCode) {
        ...
        let storageData = sessionStorage.getItem(`工序3-1批量上料`)
        if (storageData) {
      ...
      this.fetchEqpList() // 获取退火炉list
      getSeleteData('process3-1Tec', this.technologyList) // 工艺list
    },
    reset() {
    ...
    async handle(typeName = '') {
      if (typeName === '保存') {
        ...
        sessionStorage.setItem(`工序3-1批量上料`, JSON.stringify(saveData))
        ...
        this.$refs.detailForm.validate((valid) => {
          if (valid) {
            ...
            this.$confirm('确认进行批量上料?', '提示', {
              ...
              sessionStorage.removeItem(`工序3-1批量上料`)
    ...
    async fetchInfo(index = 0, showMsg = true) {
      ...
      if (this.isExist(uniqueCode, index)) {
        ...
        return this.$message.warning(uniqueCode + '已存在, 请不要重复录入!')
      }
      ...
      if (!res.data) {
        this.detailForm.list[index].uniqueCode = ''
        return this.$message.warning(`${uniqueCode} 不是工序3-1待进站状态!`)
      }
    ...
    openPosition(index) {
      if (!this.detailForm.equipmentCode) return this.$message.error('请先选择退火炉编号!')
